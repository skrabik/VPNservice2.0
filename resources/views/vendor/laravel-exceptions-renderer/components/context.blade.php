...
<x-laravel-exceptions-renderer::card class="mt-6 overflow-x-auto">
    ...
    @php($__requestHeaders = $exception->requestHeaders())
    <dl class="mt-1 grid grid-cols-1 rounded border dark:border-gray-800">
        @if (count($__requestHeaders) === 0)
            <span
        ...
        @else
            @foreach ($__requestHeaders as $key => $value)
...
<x-laravel-exceptions-renderer::card class="mt-6 overflow-x-auto">
    ...
    <dl class="mt-1 grid grid-cols-1 rounded border dark:border-gray-800">
        ...
        @else
            @foreach ($__routeContext as $name => $value)
                <div class="flex items-center gap-2 {{ $loop->first ? '' : 'border-t' }} dark:border-gray-800">
                    <span
    ...
    <dl class="mt-1 grid grid-cols-1 rounded border dark:border-gray-800">
        ...
        @else
            @foreach ($__queries as ['connectionName' => $connectionName, 'sql' => $sql, 'time' => $time])
                <div class="flex items-center gap-2 {{ $loop->first ? '' : 'border-t' }} dark:border-gray-800">
